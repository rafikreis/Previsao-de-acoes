...
# Momentum
df['Sk1_momentum'] = df['Stock1'].diff(4) 
...
df['Sk1_volatility'] = df['Sk1Pct'].rolling(window=30).std() * np.sqrt(252) 
df['Sk2_volatility'] = df['Sk2Pct'].rolling(window=30).std() * np.sqrt(252) 
...
# model1 = load_model('AAAA.keras')

# predictions1 = model1.predict(XL)[:, 1]

# df_logic['PredictionLast'] = predictions1
...
while(True):
    ...
    def custom_predict(predictions):
        return np.where(predictions > 0.51, 1, np.where(predictions < 0.49, -1, 0))
    

    XL_train, XL_test, yL_train, yL_test = train_test_split(XL, yL, test_size=0.35, random_state=42)

    pca = PCA(n_components='mle')
    principalComponents = pca.fit_transform(XL)

    model = Sequential([
        Dense(64, activation='relu', input_shape=(XL_train.shape[1],)),
    ...
    model.fit(XL_train, yL_train, epochs=50, batch_size=32, validation_data=(XL_test, yL_test))
    
    predictions = model.predict(XL)[:, 1]
    predictionsF = custom_predict(predictions)

    df_resultlog = df[['Stock1','Stock2','Sk1Acc', 'Sk2Acc', 'Sk1PctTwo', 'Sk2PctTwo','Ratio']]
    df_resultlog['RatioPct'] = df['Ratio'].pct_change(periods=2)
    df_resultlog['PositionLog'] = predictionsF
    ...
    if(accuracy > 64 and anual > 78):
        break


save = 1

if accuracy > 64 and anual > 78 and save == 1:
    model.save("MeuModelo_GOGL34xPETR4.keras")
